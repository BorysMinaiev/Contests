...
public class SolveNew3 {
	...
	final int OUTER_ITER = 100;
	final int INNER_ITER = 10;
	...
	void solve(int addRouters) {
		...
		ArrayList<Answer> answers = new ArrayList<SolveNew3.Answer>();
		...
		for (int ITER = 0; ITER < answers.size() && ITER < INNER_ITER; ITER++) {
			...
			while (firstLocalOpt(true) || thirdLocalOpt()) {
			}
	...
	class Answer implements Comparable<Answer> {
		...
		public Answer(int score) {
			super();
			this.score = score;
			this.points = new ArrayList<SolveNew3.Point>();
	...
	boolean firstLocalOpt(boolean debug) {
		boolean allCh = false;
		while (true) {
			if (debug)
	...
	boolean secondLocalOpt() {
		boolean allCh = false;
		while (true) {
			ArrayList<P> check = new ArrayList<P>();
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < m; j++) {
					if (routerHere[i][j]) {
						notUseXY(i, j);
	...
	boolean thirdLocalOpt() {
		boolean allCh = false;
		while (true) {

			boolean ch = false;

			int myScore = coveredByRouters;
			System.err.println("from " + myScore);
			Answer currentAnswer = genAnswer();
			...
			if (coveredByRouters > myScore) {
				System.err.println("third opt ok -> " + coveredByRouters);
				ch = true;
			} else {
				restoreAnswer(currentAnswer);
			}
